...
// 登录校验成功后签发jwt
func (b *BaseApi) tokenNext(ctx *gin.Context, user model.SysUser) {
	// 生成Token
	...
	menus = append(menus, response.Menus{
		Path:   "首页",
		Title:  "首页",
		Header: "首页",
	})
	...
	response.OkWithDataAndMsg(ctx, response.LoginResponse{
		Token:     token,
		UserInfo:  response.UserInfo{Id: user.UUID, Account: user.Username, HeadPic: ""},
...
func (b *BaseApi) GetUserList(c *gin.Context) {
	...
	response.OkWithDataAndMsg(c, response.PageResult[model.SysUser]{
		List:     list,
		Total:    total,
		Page:     pageInfo.Page,
